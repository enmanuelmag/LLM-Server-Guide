...
const upload = multer({
  ...
  fileFilter: (req, file, cb) => {
    const allowedMimes = ['audio/mpeg', 'audio/wav', 'audio/mp4', 'audio/flac', 'audio/x-m4a'];
...
router.post('/query-voice-to-text', upload.single('audio'), async (req, res) => {
  ...
  try {
    ...
    Logger.info(`Processing audio file: ${req.file.originalname} (${req.file.size} bytes)`);
...
router.post('/query-voice-to-voice', upload.single('audio'), async (req, res) => {
  let audioFilePath: string | undefined;
