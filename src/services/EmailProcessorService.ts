...
export class EmailProcessorService {
  ...
  constructor(ragService: RAGService, fineTuningService: FineTuningService) {
  ...
  async processEmail(
    ...
    try {
      // 1. Classify if email is financial using fine-tuning
      const classification =
        await this.fineTuningService.classifyWithFineTunedModel({
      ...
      if (!classification.isFinancial || classification.confidence < 0.6) {
        Logger.debug(
          `⏭️ Email not financial or low confidence: ${classification.confidence}`
        );
        return null;
      }

      // 2. Use RAG to check against company policies
      const policyQuery = `Email from ${email.from}: ${
        email.subject
      }. Amount: ${classification.extractedAmount?.value || 'unknown'}`;
      const ragResult = await this.ragService.answerQuery(policyQuery);

      // 3. Extract financial details with tool calling
      const extractionResult = await this.extractFinancialDetails(
        email,
        ragResult.answer
      );
      ...
      Logger.success(
        `✅ Email processed successfully: ${validatedEvent.name} ($${validatedEvent.amount.value})`
      );
   ...
   */
  private async extractFinancialDetails(
    email: EmailData,
    ...
    const completion = await this.openai.chat.completions.create({
      ...
      messages: [
        { role: 'system', content: systemPrompt },
        {
          role: 'user',
          content:
            'Analiza este email y extrae la información financiera usando la función correspondiente.',
        },
      ],
      tools,
      tool_choice: 'auto',
