...
async function startServer() {
  try {
    ...
    app.listen(config.server.port, () => {
      ...
      Logger.info(`📚 Workshop branch: 5-fetch-emails`);
      Logger.info(`🔗 Visit: http://localhost:${config.server.port}`);
      Logger.info(
        `📖 Documentation: http://localhost:${config.server.port}/docs`
      );
      Logger.info(
        `🧠 Try Enhanced RAG: POST /rag/query with {"query": "¿cuánto gasté en comestibles?"}`
      );
      Logger.info(
        `🔍 Search Options: GET /rag/search-options for available parameters`
      );
      Logger.info(
        `🎯 Next: Switch to branch 6-api-moderation for content moderation`
      );
    });
  } catch (error) {
